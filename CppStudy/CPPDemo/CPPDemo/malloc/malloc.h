...
typedef struct _malloc_zone_t {
    ...
    // 只有zone实现者才能依赖于此结构的布局； 普通调用者应该使用下面的访问函数
    void    *reserved1;    /* RESERVED FOR CFAllocator DO NOT USE  为CFAllocator保留 不使用 */
    void    *reserved2;    /* RESERVED FOR CFAllocator DO NOT USE  为CFAllocator保留 不使用 */
    size_t     (* MALLOC_ZONE_FN_PTR(size))(struct _malloc_zone_t *zone, const void *ptr); /* returns the size of a block or 0 if not in this zone; must be fast, especially for negative answers */ // 返回块大小，如果不在此zone中则返回0； 特别是对于否定的响应，必须快速
    void     *(* MALLOC_ZONE_FN_PTR(malloc))(struct _malloc_zone_t *zone, size_t size);
    void     *(* MALLOC_ZONE_FN_PTR(calloc))(struct _malloc_zone_t *zone, size_t num_items, size_t size); /* same as malloc, but block returned is set to zero */ // 与malloc相同，但是返回的块设置为0
    void     *(* MALLOC_ZONE_FN_PTR(valloc))(struct _malloc_zone_t *zone, size_t size); /* same as malloc, but block returned is set to zero and is guaranteed to be page aligned */ // 与malloc相同，但是返回的块设置为0，并保证页面对齐
    void     (* MALLOC_ZONE_FN_PTR(free))(struct _malloc_zone_t *zone, void *ptr);
    void     *(* MALLOC_ZONE_FN_PTR(realloc))(struct _malloc_zone_t *zone, void *ptr, size_t size);
    void     (* MALLOC_ZONE_FN_PTR(destroy))(struct _malloc_zone_t *zone); /* zone is destroyed and all memory reclaimed */ // zone被销毁，并且所有内存被回收
    const char    *zone_name; // zone的名称

    /* Optional batch callbacks; these may be NULL */ // 可选的批量回调；这些回调可以是 NULL
    unsigned    (* MALLOC_ZONE_FN_PTR(batch_malloc))(struct _malloc_zone_t *zone, size_t size, void **results, unsigned num_requested); /* given a size, returns pointers capable of holding that size; returns the number of pointers allocated (maybe 0 or less than num_requested) */ // 如果指定了尺寸，则返回能够保持该尺寸的指针; 返回分配的指针数（可能小于 0）
    void    (* MALLOC_ZONE_FN_PTR(batch_free))(struct _malloc_zone_t *zone, void **to_be_freed, unsigned num_to_be_freed); /* frees all the pointers in to_be_freed; note that to_be_freed may be overwritten during the process */ // 释放所有在 to_be_freed 中的指针；请注意 在进程中 to_be_freed 可能被重写

    // 每个 malloc 区域都包含一个指向 malloc_introspection_t 的指针。暴露 enumerator 的结构函数可用于发现已被 malloc 区域保留的内存区域，以及这些区域内所有单独分配的详细信息
    struct malloc_introspection_t    * MALLOC_INTROSPECT_TBL_PTR(introspect);
    unsigned    version;
        
    /* aligned memory allocation. The callback may be NULL. Present in version >= 5. */
    // 对齐内存分配。回调可能为NULL。version大于等于5后出现的
    void *(* MALLOC_ZONE_FN_PTR(memalign))(struct _malloc_zone_t *zone, size_t alignment, size_t size);
    
    /* free a pointer known to be in zone and known to have the given size. The callback may be NULL. Present in version >= 6.*/ // 释放已知在zone内，并且已知具有特定大小的指针。
    void (* MALLOC_ZONE_FN_PTR(free_definite_size))(struct _malloc_zone_t *zone, void *ptr, size_t size);

    /* Empty out caches in the face of memory pressure. The callback may be NULL. Present in version >= 8. */ // 在面临内存压力时清空高速缓存
