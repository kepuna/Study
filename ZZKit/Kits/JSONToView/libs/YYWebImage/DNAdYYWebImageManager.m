...
@implementation DNAdYYWebImageManager

+ (instancetype)sharedManager {
    static DNAdYYWebImageManager *manager;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        DNAdYYImageCache *cache = [DNAdYYImageCache sharedCache];
...
- (instancetype)initWithCache:(DNAdYYImageCache *)cache queue:(NSOperationQueue *)queue{
    self = [super init];
    if (!self) return nil;
    _cache = cache;
    _queue = queue;
...
- (DNAdYYWebImageOperation *)requestImageWithURL:(NSURL *)url
    ...
    DNAdYYWebImageOperation *operation = [[DNAdYYWebImageOperation alloc] initWithRequest:request
                                                                          options:options
                                                                            cache:_cache
