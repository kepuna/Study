...
@interface DNAdYYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) DNAdYYImageDecoder *progressiveDecoder;
...
@implementation DNAdYYWebImageOperation
    ...
    if ([self isExecuting]) {
        ...
        if (_connection) {
            ...
            if (![_request.URL isFileURL] && (_options & YYWebImageOptionShowNetworkActivity)) {
                [DNAdYYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            ...
            UIImage *image = [_cache getImageForKey:_cacheKey withType:DNAdYYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
            ...
            if (!(_options & YYWebImageOptionIgnoreDiskCache)) {
                ...
                dispatch_async([self.class _imageQueue], ^{
                    ...
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:DNAdYYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:DNAdYYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            if (![_request.URL isFileURL] && (_options & YYWebImageOptionShowNetworkActivity)) {
                [DNAdYYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([DNAdYYWebImageOperation _imageQueue], ^{
                        DNAdYYImageCacheType cacheType = (_options & YYWebImageOptionIgnoreDiskCache) ? DNAdYYImageCacheTypeMemory : DNAdYYImageCacheTypeAll;
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[DNAdYYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == DNAdYYImageTypeUnknown ||
            _progressiveDecoder.type == DNAdYYImageTypeWebP ||
            _progressiveDecoder.type == DNAdYYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != DNAdYYImageTypeJPEG &&
                _progressiveDecoder.type != DNAdYYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            DNAdYYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
            ...
            } else if (_progressiveDecoder.type == DNAdYYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                if (allowAnimation) {
                    image = [[DNAdYYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image yy_imageByDecoded];
                    if ([((DNAdYYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    DNAdYYImageDecoder *decoder = [DNAdYYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                DNAdYYImageType imageType = DNAdYYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case DNAdYYImageTypeJPEG:
                    case DNAdYYImageTypeGIF:
                    case DNAdYYImageTypePNG:
                    case DNAdYYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == DNAdYYImageTypeGIF ||
                                imageType == DNAdYYImageTypeWebP) {
