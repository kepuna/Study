...
static DNAdYYDiskCache *_YYDiskCacheGetGlobal(NSString *path) {
...
static void _YYDiskCacheSetGlobal(DNAdYYDiskCache *cache) {
...
@implementation DNAdYYDiskCache {
    DNAdYYKVStorage *_kv;
...
- (instancetype)initWithPath:(NSString *)path
    ...
    self = [super init];
    if (!self) return nil;
    // // 先从 NSMapTable 单例中根据 path 获取 YYDiskCache 实例
    DNAdYYDiskCache *globalCache = _YYDiskCacheGetGlobal(path);
    if (globalCache) return globalCache;
    
    // 没有获取到则初始化一个 YYDiskCache 实例
    // 要想初始化一个 YYDiskCache 首先要初始化一个 YYKVStorage
    DNAdYYKVStorageType type;
    if (threshold == 0) {
        type = DNAdYYKVStorageTypeFile; // 存储到file
    } else if (threshold == NSUIntegerMax) {
        type = DNAdYYKVStorageTypeSQLite; // 存储到数据库
    } else {
        type = DNAdYYKVStorageTypeMixed; // 混合存储
    }
    
    DNAdYYKVStorage *kv = [[DNAdYYKVStorage alloc] initWithPath:path type:type];
    if (!kv) return nil;
    ...
    [self _trimRecursively];  // 开启递归定时清理
    _YYDiskCacheSetGlobal(self); // 向 NSMapTable 单例注册新生成的 YYDiskCache 实例
...
- (id<NSCoding>)objectForKey:(NSString *)key {
    if (!key) return nil;
    Lock();
    DNAdYYKVStorageItem *item = [_kv getItemForKey:key];
    Unlock();
    ...
    if (object && item.extendedData) {
        [DNAdYYDiskCache setExtendedData:item.extendedData toObject:object];
    }
    return object;
}
...
- (void)setObject:(id<NSCoding>)object forKey:(NSString *)key {
    ...
    NSData *extendedData = [DNAdYYDiskCache getExtendedDataFromObject:object];
    NSData *value = nil;
    ...
    if (!value) return;
    NSString *filename = nil;
    if (_kv.type != DNAdYYKVStorageTypeSQLite) {
