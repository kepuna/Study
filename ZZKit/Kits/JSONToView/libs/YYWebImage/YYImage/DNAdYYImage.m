...
@implementation DNAdYYImage {
    DNAdYYImageDecoder *_decoder;
    NSArray *_preloadedFrames;
    dispatch_semaphore_t _preloadedLock;
    NSUInteger _bytesPerFrame;
}

+ (DNAdYYImage *)imageNamed:(NSString *)name {
...
+ (DNAdYYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (DNAdYYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (DNAdYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        DNAdYYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
