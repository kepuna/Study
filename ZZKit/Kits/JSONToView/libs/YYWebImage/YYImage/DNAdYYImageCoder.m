...
DNAdYYImageType DNAdYYImageDetectType(CFDataRef data) {
    if (!data) return DNAdYYImageTypeUnknown;
    uint64_t length = CFDataGetLength(data);
    if (length < 16) return DNAdYYImageTypeUnknown;
    ...
    switch (magic4) {
        ...
        case YY_FOUR_CC('i', 'c', 'n', 's'): { // ICNS
            return DNAdYYImageTypeICNS;
        } break;
            
        case YY_FOUR_CC('G', 'I', 'F', '8'): { // GIF
            return DNAdYYImageTypeGIF;
        } break;
            
        case YY_FOUR_CC(0x89, 'P', 'N', 'G'): {  // PNG
            uint32_t tmp = *((uint32_t *)(bytes + 4));
            if (tmp == YY_FOUR_CC('\r', '\n', 0x1A, '\n')) {
                return DNAdYYImageTypePNG;
            }
        } break;
        ...
        case YY_FOUR_CC('R', 'I', 'F', 'F'): { // WebP
            ...
            if (tmp == YY_FOUR_CC('W', 'E', 'B', 'P')) {
                return DNAdYYImageTypeWebP;
            }
        } break;
        /*
        case YY_FOUR_CC('B', 'P', 'G', 0xFB): { // BPG
            return DNAdYYImageTypeBPG;
        } break;
        */
    }
    ...
    switch (magic2) {
        ...
        case YY_TWO_CC('C', 'P'): { // BMP
            return DNAdYYImageTypeBMP;
        }
        case YY_TWO_CC(0xFF, 0x4F): { // JPEG2000
            return DNAdYYImageTypeJPEG2000;
        }
    }
    
    // JPG             FF D8 FF
    if (memcmp(bytes,"\377\330\377",3) == 0) return DNAdYYImageTypeJPEG;
    
    // JP2
    if (memcmp(bytes + 4, "\152\120\040\040\015", 5) == 0) return DNAdYYImageTypeJPEG2000;
    
    return DNAdYYImageTypeUnknown;
}

CFStringRef DNAdYYImageTypeToUTType(DNAdYYImageType type) {
    switch (type) {
        case DNAdYYImageTypeJPEG: return kUTTypeJPEG;
        case DNAdYYImageTypeJPEG2000: return kUTTypeJPEG2000;
        case DNAdYYImageTypeTIFF: return kUTTypeTIFF;
        case DNAdYYImageTypeBMP: return kUTTypeBMP;
        case DNAdYYImageTypeICO: return kUTTypeICO;
        case DNAdYYImageTypeICNS: return kUTTypeAppleICNS;
        case DNAdYYImageTypeGIF: return kUTTypeGIF;
        case DNAdYYImageTypePNG: return kUTTypePNG;
        default: return NULL;
    }
}

DNAdYYImageType DNAdYYImageTypeFromUTType(CFStringRef uti) {
    static NSDictionary *dic;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        dic = @{(id)kUTTypeJPEG : @(DNAdYYImageTypeJPEG),
                (id)kUTTypeJPEG2000 : @(DNAdYYImageTypeJPEG2000),
                (id)kUTTypeTIFF : @(DNAdYYImageTypeTIFF),
                (id)kUTTypeBMP : @(DNAdYYImageTypeBMP),
                (id)kUTTypeICO : @(DNAdYYImageTypeICO),
                (id)kUTTypeAppleICNS : @(DNAdYYImageTypeICNS),
                (id)kUTTypeGIF : @(DNAdYYImageTypeGIF),
                (id)kUTTypePNG : @(DNAdYYImageTypePNG)};
    });
    if (!uti) return DNAdYYImageTypeUnknown;
    NSNumber *num = dic[(__bridge __strong id)(uti)];
    return num.unsignedIntegerValue;
}

NSString *DNAdYYImageTypeGetExtension(DNAdYYImageType type) {
    switch (type) {
        case DNAdYYImageTypeJPEG: return @"jpg";
        case DNAdYYImageTypeJPEG2000: return @"jp2";
        case DNAdYYImageTypeTIFF: return @"tiff";
        case DNAdYYImageTypeBMP: return @"bmp";
        case DNAdYYImageTypeICO: return @"ico";
        case DNAdYYImageTypeICNS: return @"icns";
        case DNAdYYImageTypeGIF: return @"gif";
        case DNAdYYImageTypePNG: return @"png";
        case DNAdYYImageTypeWebP: return @"webp";
        default: return nil;
    }
}

CFDataRef DNAdYYCGImageCreateEncodedData(CGImageRef imageRef, DNAdYYImageType type, CGFloat quality) {
    if (!imageRef) return nil;
    quality = quality < 0 ? 0 : quality > 1 ? 1 : quality;
    
    if (type == DNAdYYImageTypeWebP) {
    ...
    CFStringRef uti = DNAdYYImageTypeToUTType(type);
    if (!uti) return nil;
...
@implementation DNAdYYImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    DNAdYYImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    DNAdYYImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _YYImageDecoderFrame : DNAdYYImageFrame
...
@implementation DNAdYYImageDecoder {
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    DNAdYYImageDecoder *decoder = [[DNAdYYImageDecoder alloc] initWithScale:scale];
...
- (DNAdYYImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    DNAdYYImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (BOOL)_updateData:(NSData *)data final:(BOOL)final {
    ...
    DNAdYYImageType type = DNAdYYImageDetectType((__bridge CFDataRef)data);
...
- (DNAdYYImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    if (index >= _frames.count) return 0;
    _YYImageDecoderFrame *frame = [(_YYImageDecoderFrame *)_frames[index] copy];
    BOOL decoded = NO;
    BOOL extendToCanvas = NO;
    if (_type != DNAdYYImageTypeICO && decodeForDisplay) { // ICO contains multi-size frame and should not extend to canvas.
        extendToCanvas = YES;
    }
    ...
    if (_blendFrameIndex + 1 == frame.index) {
        ...
        if (frame.blendFromIndex == frame.index) {
            ...
            if (frame.dispose == DNAdYYImageDisposeBackground) {
    ...
    if (extendToCanvas) {
        ...
        frame.offsetX = 0;
        frame.offsetY = 0;
        frame.dispose = DNAdYYImageDisposeNone;
        frame.blend = DNAdYYImageBlendNone;
    }
    return frame;
}
...
- (void)_updateSource {
    switch (_type) {
        case DNAdYYImageTypeWebP: {
            [self _updateSourceWebP];
        } break;
            
        case DNAdYYImageTypePNG: {
    ...
    for (uint32_t i = 0; i < apng->apng_frame_num; i++) {
        ...
        switch (fi->frame_control.dispose_op) {
            case YY_PNG_DISPOSE_OP_BACKGROUND: {
                frame.dispose = DNAdYYImageDisposeBackground;
            } break;
            case YY_PNG_DISPOSE_OP_PREVIOUS: {
                frame.dispose = DNAdYYImageDisposePrevious;
            } break;
            default: {
                frame.dispose = DNAdYYImageDisposeNone;
            } break;
        ...
        switch (fi->frame_control.blend_op) {
            case YY_PNG_BLEND_OP_OVER: {
                frame.blend = DNAdYYImageBlendOver;
            } break;
        ...
        if (frame.blend == DNAdYYImageBlendNone && frame.isFullSize) {
            frame.blendFromIndex  = i;
            if (frame.dispose != DNAdYYImageDisposePrevious) lastBlendIndex = i;
        } else {
            if (frame.dispose == DNAdYYImageDisposeBackground && frame.isFullSize) {
                frame.blendFromIndex = lastBlendIndex;
                lastBlendIndex = i + 1;
            } else {
                frame.blendFromIndex = lastBlendIndex;
...
- (void)_updateSourceImageIO {
    ...
    if (!_finalized) { // ignore multi-frame before finalized
        ...
        if (_type == DNAdYYImageTypePNG) { // use custom apng decoder and ignore multi-frame
            _frameCount = 1;
        }
        if (_type == DNAdYYImageTypeGIF) { // get gif loop count
            CFDictionaryRef properties = CGImageSourceCopyProperties(_source, NULL);
            if (properties) {
...
- (void)_blendImageWithFrame:(_YYImageDecoderFrame *)frame {
    if (frame.dispose == DNAdYYImageDisposePrevious) {
        // nothing
    } else if (frame.dispose == DNAdYYImageDisposeBackground) {
        CGContextClearRect(_blendCanvas, CGRectMake(frame.offsetX, frame.offsetY, frame.width, frame.height));
    } else { // no dispose
        if (frame.blend == DNAdYYImageBlendOver) {
...
@implementation DNAdYYImageEncoder {
...
- (instancetype)init {
    ...
    return [self initWithType:DNAdYYImageTypeUnknown];
}

- (instancetype)initWithType:(DNAdYYImageType)type {
    if (type == DNAdYYImageTypeUnknown || type >= DNAdYYImageTypeOther) {
    ...
    switch (type) {
        case DNAdYYImageTypeJPEG:
        case DNAdYYImageTypeJPEG2000: {
            _quality = 0.9;
        } break;
        case DNAdYYImageTypeTIFF:
        case DNAdYYImageTypeBMP:
        case DNAdYYImageTypeGIF:
        case DNAdYYImageTypeICO:
        case DNAdYYImageTypeICNS:
        case DNAdYYImageTypePNG: {
            _quality = 1;
            _lossless = YES;
        } break;
...
- (CGImageDestinationRef)_newImageDestination:(id)dest imageCount:(NSUInteger)count CF_RETURNS_RETAINED {
    ...
    if ([dest isKindOfClass:[NSString class]]) {
        ...
        if (url) {
            destination = CGImageDestinationCreateWithURL((CFURLRef)url, DNAdYYImageTypeToUTType(_type), count, NULL);
        }
    } else if ([dest isKindOfClass:[NSMutableData class]]) {
        destination = CGImageDestinationCreateWithData((CFMutableDataRef)dest, DNAdYYImageTypeToUTType(_type), count, NULL);
    }
    return destination;
}
...
- (void)_encodeImageWithDestination:(CGImageDestinationRef)destination imageCount:(NSUInteger)count {
    ...
    for (int i = 0; i < count; i++) {
        @autoreleasepool {
            ...
            if (_type == DNAdYYImageTypeGIF && count > 1) {
...
- (NSData *)_encodeWithImageIO {
    ...
    NSUInteger count = _type == DNAdYYImageTypeGIF ? _images.count : 1;
...
- (NSData *)_encodeAPNG {
    ...
    for (int i = 0; i < _images.count; i++) {
        ...
        CFDataRef frameData = DNAdYYCGImageCreateEncodedData(decoded, DNAdYYImageTypePNG, 1);
        CFRelease(decoded);
    ...
    if (firstFrameSize.width < canvasWidth || firstFrameSize.height < canvasHeight) {
        ...
        CFDataRef frameData = DNAdYYCGImageCreateEncodedData(extendedImage, DNAdYYImageTypePNG, 1);
        if (!frameData) {
...
- (NSData *)encode {
    ...
    if (_type == DNAdYYImageTypePNG) return [self _encodeAPNG];
    if (_type == DNAdYYImageTypeWebP) return [self _encodeWebP];
    return nil;
}
...
+ (NSData *)encodeImage:(UIImage *)image type:(DNAdYYImageType)type quality:(CGFloat)quality {
    DNAdYYImageEncoder *encoder = [[DNAdYYImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(DNAdYYImageDecoder *)decoder type:(DNAdYYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (DNAdYYImageCoder)
...
- (BOOL)yy_isDecodedForDisplay {
    if (self.images.count > 1 || [self isKindOfClass:[DNAdYYSpriteSheetImage class]]) return YES;
...
- (NSData *)_yy_dataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[DNAdYYImage class]]) {
        DNAdYYImage *image = (id)self;
        if (image.animatedImageData) {
            if (forSystem) { // system only support GIF and PNG
                if (image.animatedImageType == DNAdYYImageTypeGIF ||
                    image.animatedImageType == DNAdYYImageTypePNG) {
