...
static CGFloat const kHooDatePickerHeaderHeight = 54.0f; // 取消、确定的headerview的高度
static CGFloat const kHooDatePickerHeaderMarginHeight = 5.0f;
...
static CGFloat const kHooDatePickerScrollViewLeftMargin = 0.0f; // 年scrllView和月scrollView之前的间距

static CGFloat const kHooDatePickerScrollViewItemHeight = 45.0f;

static CGFloat const kHooDatePickerLineWidth = 0.5f;

static CGFloat const kHooDatePickerLineMargin = 0.0f; // 比如选中年月至 线和先之间的距离
...
// Constants fonts
#define kHooDatePickerTitleFont [UIFont fontWithName:@"PingFangSC-Regular" size: 16]
#define kHooDatePickerLabelFont [UIFont fontWithName:@"PingFangSC-Thin" size: 14.5]
#define kHooDatePickerLabelSelectedFont [UIFont fontWithName:@"PingFangSC-Medium" size: 16];

// Constants colors :
#define kHooDatePickerTintColor [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0]
#define kHooDatePickerHighlightColor [UIColor colorWithRed:51/255.0 green:51/255.0 blue:51/255.0 alpha:1.0]
// 整个picker的颜色
#define kHooDatePickerBackgroundColor [UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0]
#define kHooDatePickerScrolViewBackgroundColor [UIColor colorWithRed:2555.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0]
...
#define kHooDatePickerBackgroundColorLines [UIColor colorWithRed:230/255.0 green:230/255.0 blue:230/255.0 alpha:1.0]

#define kHooDatePickerHeaderMarginColor [UIColor colorWithRed:245/255.0 green:245/255.0 blue:245/255.0 alpha:1.0]
...
@property (nonatomic, strong) UIView *marginView; // 新增的mariginView
...
- (instancetype)initWithSuperView:(UIView*)superView title:(NSString *)title{
    ...
    if (self = [super initWithFrame:CGRectMake(0.0, superView.frame.size.height, superView.frame.size.width, kHooDatePickerHeight)]) {
        ...
        [self setupControl];
//        superView.backgroundColor = [UIColor colorWithRed:0/255.0 green:0/255.0 blue:0/255.0 alpha:0.6];
    }
    return self;
}
...
//    NSString * month = @"01";
    ...
    if ([self.year length]>0&&[self.month length]>0) {
        NSString * dateStr = [NSString stringWithFormat:@"01-%@-%@ 00:00:00",self.month,self.year];
        
        NSDate *Date = [dateFormatter dateFromString:dateStr];
        //    NSDate *minDate = [dateFormatter dateFromString:@"01-01-2016 00:00:00"];
        [self setDate:Date animated:YES];//设置默认日期
    }
    else
    {
        [self setDate:[NSDate date] animated:YES];//设置默认日期
    }
}
...
//    _lineMonthsTop = [[UIView alloc] initWithFrame:CGRectMake(width, _scrollViewMonths.frame.origin.y + (_scrollViewMonths.frame.size.height / 2) - (kHooDatePickerScrollViewItemHeight / 2), width, kHooDatePickerLineWidth)];
...
//    _lineMonthsBottom = [[UIView alloc] initWithFrame:CGRectMake(width, _scrollViewMonths.frame.origin.y + (_scrollViewMonths.frame.size.height / 2) + (kHooDatePickerScrollViewItemHeight / 2), width, kHooDatePickerLineWidth)];
    _lineMonthsBottom.backgroundColor = kHooDatePickerBackgroundColorLines;
...
//    [self.superView insertSubview:self.dimBackgroundView belowSubview:self];  原来的逻辑
 // 替换原来的逻辑
    [[UIApplication sharedApplication].keyWindow addSubview:self.dimBackgroundView];
    ...
    [UIView animateWithDuration:kHooDatePickerAnimationDuration delay:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        if (self.isHasTabBar) {
            //通知，隐藏tabbar
            [[NSNotificationCenter defaultCenter]postNotificationName:@"hideMyTabbar" object:nil];
        }
...
//        self.dimBackgroundView.backgroundColor = [UIColor redColor];
        //通知，出现tabbar
        if (self.isHasTabBar) {
...
- (void)setDate:(NSDate *)date animated:(BOOL)animated {
    ...
    //NSLog(@"1_selectedMonth:%ld",_selectedMonth);
    /*
     输入  显示
     12	   11
     11      10
    ...
    //此时代表存在默认值
    if ([self.year length]>0&&[self.month length]>0) {
        if (_selectedMonth >= 1&&_selectedMonth <= 11) {
            _selectedMonth += 1;
        }
        else
        {
    ...
    //无默认值时，显示的是当前日期，不需要矫正

    

    
    //NSLog(@"2_selectedMonth:%ld",_selectedMonth);
    /**
...
- (UIView *)dimBackgroundView {
    if(!_dimBackgroundView) {
        ...
        _dimBackgroundView.backgroundColor = [[UIColor darkGrayColor] colorWithAlphaComponent:0.618];
...
- (UIView *)headerView {
    if (!_headerView) {
        ...
        cancelButton.titleLabel.font = kHooDatePickerTitleFont;
        ...
        [sureButton setTitle:kSureButtonItemTitle forState:UIControlStateNormal];
         sureButton.titleLabel.font = kHooDatePickerTitleFont;
        ...
        [_headerView addSubview:self.marginView];
    }
    return _headerView;
}
...
- (UIView *)marginView {
    if (_marginView == nil) {
        CGRect frame = CGRectMake(0,kHooDatePickerHeaderHeight - kHooDatePickerHeaderMarginHeight, self.bounds.size.width, kHooDatePickerHeaderMarginHeight);
        _marginView = [[UIView alloc] initWithFrame:frame];
        _marginView.backgroundColor = kHooDatePickerHeaderMarginColor;
    }
    return _marginView;
}
...
//        NSString *stringFromDate = [formatter stringFromDate:myDate];
        NSString *stringFromDate = [NSString stringWithFormat:@"%d月",monthNumber];
