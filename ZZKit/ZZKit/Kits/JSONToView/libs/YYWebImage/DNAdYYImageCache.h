...
@class DNAdYYMemoryCache, DNAdYYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, DNAdYYImageCacheType) {
    /// No value.
    DNAdYYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    DNAdYYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    DNAdYYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    DNAdYYImageCacheTypeAll    = DNAdYYImageCacheTypeMemory | DNAdYYImageCacheTypeDisk,
};
 ...
 */
@interface DNAdYYImageCache : NSObject
...
@property (strong, readonly) DNAdYYMemoryCache *memoryCache;

/** The underlying disk cache. see `YYDiskCache` for more information.*/
@property (strong, readonly) DNAdYYDiskCache *diskCache;
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(DNAdYYImageCacheType)type;
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(DNAdYYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(DNAdYYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, DNAdYYImageCacheType type))block;
