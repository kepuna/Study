...
@implementation DNAdYYKVStorageItem
@end

@implementation DNAdYYKVStorage {
...
- (DNAdYYKVStorageItem *)_dbGetItemFromStmt:(sqlite3_stmt *)stmt excludeInlineData:(BOOL)excludeInlineData {
    int i = 0;
    ...
    DNAdYYKVStorageItem *item = [DNAdYYKVStorageItem new];
...
- (DNAdYYKVStorageItem *)_dbGetItemWithKey:(NSString *)key excludeInlineData:(BOOL)excludeInlineData {
    NSString *sql = excludeInlineData ? @"select key, filename, size, modification_time, last_access_time, extended_data from manifest where key = ?1;" : @"select key, filename, size, inline_data, modification_time, last_access_time, extended_data from manifest where key = ?1;";
    sqlite3_stmt *stmt = [self _dbPrepareStmt:sql];
    if (!stmt) return nil;
    sqlite3_bind_text(stmt, 1, key.UTF8String, -1, NULL);
    
    DNAdYYKVStorageItem *item = nil;
    int result = sqlite3_step(stmt);
...
- (NSMutableArray *)_dbGetItemWithKeys:(NSArray *)keys excludeInlineData:(BOOL)excludeInlineData {
    ...
    do {
        result = sqlite3_step(stmt);
        if (result == SQLITE_ROW) {
            DNAdYYKVStorageItem *item = [self _dbGetItemFromStmt:stmt excludeInlineData:excludeInlineData];
...
- (instancetype)init {
    ...
    return [self initWithPath:@"" type:DNAdYYKVStorageTypeFile];
}

- (instancetype)initWithPath:(NSString *)path type:(DNAdYYKVStorageType)type {
    if (path.length == 0 || path.length > kPathLengthMax) {
        NSLog(@"YYKVStorage init error: invalid path: [%@].", path);
        return nil;
    }
    if (type > DNAdYYKVStorageTypeMixed) {
...
- (BOOL)saveItem:(DNAdYYKVStorageItem *)item {
...
- (BOOL)saveItemWithKey:(NSString *)key value:(NSData *)value filename:(NSString *)filename extendedData:(NSData *)extendedData {
    ...
    if (_type == DNAdYYKVStorageTypeFile && filename.length == 0) {
        return NO;
    }
    ...
    if (filename.length) {
        ...
        if (_type != DNAdYYKVStorageTypeSQLite) {
...
- (BOOL)removeItemForKey:(NSString *)key {
    ...
    switch (_type) {
        case DNAdYYKVStorageTypeSQLite: {
            return [self _dbDeleteItemWithKey:key];
        } break;
        case DNAdYYKVStorageTypeFile:
        case DNAdYYKVStorageTypeMixed: {
            NSString *filename = [self _dbGetFilenameWithKey:key];
            if (filename) {
                [self _fileDeleteWithName:filename];
            }
...
- (BOOL)removeItemsToFitSize:(int)maxSize {
    ...
    do {
        ...
        for (DNAdYYKVStorageItem *item in items) {
            if (total > maxSize) {
                if (item.filename) {
...
- (DNAdYYKVStorageItem *)getItemForKey:(NSString *)key {
    if (key.length == 0) return nil;
    DNAdYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:NO];
    if (item) {
...
- (DNAdYYKVStorageItem *)getItemInfoForKey:(NSString *)key {
    if (key.length == 0) return nil;
    DNAdYYKVStorageItem *item = [self _dbGetItemWithKey:key excludeInlineData:YES];
    return item;
}
...
- (NSArray *)getItemForKeys:(NSArray *)keys {
    ...
    if (_type != DNAdYYKVStorageTypeSQLite) {
        for (NSInteger i = 0, max = items.count; i < max; i++) {
            DNAdYYKVStorageItem *item = items[i];
            if (item.filename) {
                item.value = [self _fileReadWithName:item.filename];
                if (!item.value) {
