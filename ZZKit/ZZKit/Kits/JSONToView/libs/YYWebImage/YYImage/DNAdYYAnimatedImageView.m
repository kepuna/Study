...
#import "DNAdYYAnimatedImageView.h"
...
@interface DNAdYYAnimatedImageView() {
    @package
    UIImage <DNAdYYAnimatedImage> *_curAnimatedImage;
...
@property (nonatomic, weak) DNAdYYAnimatedImageView *view;
@property (nonatomic, assign) NSUInteger nextIndex;
@property (nonatomic, strong) UIImage <DNAdYYAnimatedImage> *curImage;
@end

@implementation _YYAnimatedImageViewFetchOperation
- (void)main {
    __strong DNAdYYAnimatedImageView *view = _view;
    if (!view) return;
...
@implementation DNAdYYAnimatedImageView
...
- (void)imageChanged {
    ...
    if ([newVisibleImage isKindOfClass:[UIImage class]] &&
        [newVisibleImage conformsToProtocol:@protocol(DNAdYYAnimatedImage)]) {
        newImageFrameCount = ((UIImage<DNAdYYAnimatedImage> *) newVisibleImage).animatedImageFrameCount;
        if (newImageFrameCount > 1) {
            hasContentsRect = [((UIImage<DNAdYYAnimatedImage> *) newVisibleImage) respondsToSelector:@selector(animatedImageContentsRectAtIndex:)];
        }
    }
    ...
    if (hasContentsRect) {
        CGRect rect = [((UIImage<DNAdYYAnimatedImage> *) newVisibleImage) animatedImageContentsRectAtIndex:0];
...
- (void)step:(CADisplayLink *)link {
    UIImage <DNAdYYAnimatedImage> *image = _curAnimatedImage;
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    ...
    BOOL ani, multi;
    ani = [self.image conformsToProtocol:@protocol(DNAdYYAnimatedImage)];
    multi = (ani && ((UIImage <DNAdYYAnimatedImage> *)self.image).animatedImageFrameCount > 1);
    if (multi) [aCoder encodeObject:self.image forKey:@"YYAnimatedImage"];
    
    ani = [self.highlightedImage conformsToProtocol:@protocol(DNAdYYAnimatedImage)];
    multi = (ani && ((UIImage <DNAdYYAnimatedImage> *)self.highlightedImage).animatedImageFrameCount > 1);
